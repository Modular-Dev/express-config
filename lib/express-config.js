...
const AssetManager = require('../config-manager/asset-manager');
const ApiRouteManager = require('../config-manager/api-route-manager');
  ...
  class ExpressConfig {
    ...
    configure(configurationType) {
      ...
      const configurationSwitch = (configType) => ({
        ...
        "assets": AssetManager.load({app, config}),
        "api": ApiRouteManager.load({app, config})
      })[configType]
